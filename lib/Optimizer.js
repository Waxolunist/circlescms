...
Optimizer.prototype.canOptimize = function (target) {
    if (typeof target !== 'string') {
        target = escodegen.generate(target, _escodegenOpts);
    }
...
Optimizer.prototype.optimizeClass = function (ast) {
    ...
    if (type === 'extend') {
        parent = escodegen.generate(ast.callee.object, _escodegenOpts);
    } else {
...
Optimizer.prototype._replaceSpecial = function (funcName, ast, isStatic) {
    var code = escodegen.generate(ast.value || ast.object, _escodegenOpts),
        newAst,
...
Optimizer.prototype._getExtends = function (ast) {
    ...
    for (x = 0; x < length; x += 1) {
        if (ast.properties[x].key.name === '$extends') {
            return escodegen.generate(ast.properties[x].value, _escodegenOpts);
        }
    }
